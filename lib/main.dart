...
class _DashboardPageState extends State<DashboardPage> {
  ...
  double? get requiredMonthAht {
    if (monthCalls <= 0 || monthRemainingCalls <= 0) return null;
    return (targetSeconds * (monthCalls + monthRemainingCalls) - monthSeconds) / monthRemainingCalls;
  }
  ...
  Future<void> _save() => LocalStorage.save(
        calls: calls,
        targetSeconds: targetSeconds,
        expectedRemainingCalls: expectedRemainingCalls,
        monthCalls: monthCalls,
        monthSeconds: monthSeconds,
        monthRemainingCalls: monthRemainingCalls,
      );
  ...
  Future<void> addCall() async {
    ...
    if (duration != null) {
      setState(() { calls.add(duration); monthCalls++; monthSeconds += duration; });
      await _save();
    }
  }
  ...
  Future<void> editPlan() async {
    ...
    final result = await showDialog<List<int>>(context: context, builder: (context) => AlertDialog(
      ...
      content: Column(mainAxisSize: MainAxisSize.min, children: [
        Row(children: [
          Expanded(child: TextField(controller: targetMin, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Target minutes'))),
          const SizedBox(width: 12),
      ...
      actions: [
        ...
        FilledButton(onPressed: () {
          ...
          final left = int.tryParse(remaining.text) ?? 0;
          if (target > 0 && left >= 0) Navigator.pop(context, [target, left]);
  ...
  Future<void> editMonth() async {
    ...
    final result = await showDialog<List<int>>(context: context, builder: (context) => AlertDialog(
      ...
      content: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [
        TextField(controller: count, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Calls handled this month')),
        const SizedBox(height: 12),
        Row(children: [
          Expanded(child: TextField(controller: ahtMin, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Current AHT min'))),
          const SizedBox(width: 12),
          Expanded(child: TextField(controller: ahtSec, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Sec'))),
        ]),
        const SizedBox(height: 12),
        TextField(controller: remaining, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Expected remaining calls this month')),
      ])),
      actions: [
        TextButton(onPressed: () => Navigator.pop(context), child: const Text('Cancel')),
        FilledButton(onPressed: () {
          final c = int.tryParse(count.text) ?? 0;
          final avg = ((int.tryParse(ahtMin.text) ?? 0) * 60) + (int.tryParse(ahtSec.text) ?? 0);
          final left = int.tryParse(remaining.text) ?? 0;
          if (c >= 0 && avg >= 0 && left >= 0) Navigator.pop(context, [c, c * avg, left]);
        }, child: const Text('Save Month')),
      ],
    ));
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('AHT Pulse', style: TextStyle(fontWeight: FontWeight.bold)), actions: [IconButton(onPressed: editPlan, icon: const Icon(Icons.tune))]),
...
class _AhtCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Card(child: Padding(padding: const EdgeInsets.all(24), child: Column(children: [Text(title), const SizedBox(height: 8), Text(value, style: const TextStyle(fontSize: 48, fontWeight: FontWeight.bold)), Text(subtitle)])));
}
...
class _MetricCard extends StatelessWidget {
  ...
  final IconData icon;
  @override
  Widget build(BuildContext context) => Card(child: Padding(padding: const EdgeInsets.all(18), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Icon(icon), const SizedBox(height: 18), Text(value, style: const TextStyle(fontSize: 25, fontWeight: FontWeight.bold)), Text(label)])));
}
